...
export default function SearchForm() {
  ...
  useEffect(() => {
    fetch('/api/sido')
      ...
      .catch(() => setGlobalError('시/도 목록을 불러오지 못했습니다.'));
  }, []);
  ...
  async function runAdminAction(action: 'region' | 'rides') {
    ...
    try {
      const endpoint = action === 'region' ? '/api/admin/refresh-region' : '/api/admin/refresh-rides';
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-900">
        <h2 className="mb-2 text-base font-bold">사용 순서 (v1)</h2>
        <ol className="list-decimal space-y-1 pl-5">
          <li>시/도를 선택합니다.</li>
          <li>시/군/구를 선택합니다. (선택)</li>
          <li>캐시가 비어 있으면 운영 패널에서 캐시를 빌드합니다.</li>
          <li>검색 실행 버튼으로 추천 결과를 확인합니다.</li>
        </ol>
      </section>

      <section className="rounded-xl bg-white p-4 shadow">
        <h2 className="mb-2 text-lg font-bold">진단 상태 요약</h2>
        <div className="grid gap-2 text-sm md:grid-cols-3">
          <StatusBadge label="Firebase" ok={status?.firebase.ok ?? false} okText="연결됨" failText="오류" />
          <StatusBadge label="공공데이터 API" ok={status?.publicApi.ok ?? false} okText="응답 가능" failText="오류" />
          <StatusBadge label="캐시" ok={(status?.counts.facilities ?? 0) > 0} okText="준비됨" failText="미준비" />
        </div>
      ...
      <section className="rounded-xl bg-white p-4 shadow">
        ...
        <div className="grid gap-2 text-sm md:grid-cols-2">
          <p>facilities 문서 수: <b>{status?.counts.facilities ?? 0}</b></p>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button onClick={() => runAdminAction('region')} disabled={runningAdminAction !== ''} className="rounded bg-slate-800 px-3 py-2 text-sm font-semibold text-white disabled:bg-slate-400">
            {runningAdminAction === 'region' ? '지역 캐시 빌드 중...' : '선택 지역 캐시 빌드'}
          </button>
          <button onClick={() => runAdminAction('rides')} disabled={runningAdminAction !== ''} className="rounded bg-indigo-700 px-3 py-2 text-sm font-semibold text-white disabled:bg-slate-400">
            {runningAdminAction === 'rides' ? 'ride 캐시 갱신 중...' : '선택 지역 ride 캐시 갱신'}
          </button>
          <button onClick={refreshStatus} disabled={loadingStatus} className="rounded border px-3 py-2 text-sm">
            상태 새로고침
          </button>
        </div>
      </section>
      ...
      <section className="rounded-xl bg-white p-4 shadow">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <select className="rounded border p-2" value={sido} onChange={(e) => setSido(e.target.value)}>
            <option value="">시/도 선택</option>
            {sidoList.map((x) => <option key={x} value={x}>{x}</option>)}
          </select>
          <select className="rounded border p-2" value={sigungu} onChange={(e) => setSigungu(e.target.value)} disabled={!sido || sigunguList.length === 0}>
        ...
        {sigunguMessage && <p className="mt-2 text-xs text-slate-600">{sigunguMessage}</p>}
        ...
        <button onClick={onSearch} disabled={loadingSearch || !sido} className="mt-4 rounded bg-blue-600 px-4 py-2 font-semibold text-white disabled:bg-slate-400">
          {loadingSearch ? '검색 중...' : '검색 실행'}
        </button>
      </section>
      ...
      {!data && !loadingSearch && (
        <section className="rounded-xl border border-dashed bg-white p-4 text-sm text-slate-600">
          아직 검색 결과가 없습니다. 위 순서대로 지역 선택 → 필요 시 캐시 빌드 → 검색 실행을 진행하세요.
        </section>
      )}

      {data && data.summary.totalCandidates === 0 && (
        <section className="rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
          {data.message ?? '조건에 맞는 결과가 없습니다.'}
        </section>
      )}
...
function StatusBadge({ label, ok, okText, failText }: { label: string; ok: boolean; okText: string; failText: string }) {
  return (
    <div className={`rounded border p-2 ${ok ? 'border-green-200 bg-green-50 text-green-800' : 'border-red-200 bg-red-50 text-red-700'}`}>
...
function ResultTable({ rows }: { rows: SearchResult[] }) {
  if (rows.length === 0) {
    return <p className="mt-2 text-sm text-slate-500">결과 없음</p>;
  }
      ...
      <table className="min-w-full text-xs">
        <thead>
          <tr className="border-b bg-slate-50 text-left">
            <th className="p-2">시설</th><th className="p-2">점수</th><th className="p-2">추천</th><th className="p-2">점수 내역</th><th className="p-2">추천 이유</th><th className="p-2">데이터주의 태그</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
