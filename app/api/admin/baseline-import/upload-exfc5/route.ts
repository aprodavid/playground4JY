...
export async function POST(req: Request) {
  try {
    ...
    await saveUploadRows('exfc5', rows);
    return jsonOk({ message: 'exfc5 파일 업로드가 완료되었습니다.', uploadedRows: rows.length, sourceFile: file.name });
  } catch (error) {
    return jsonError('exfc5 upload failed', { status: 500, detailMessage: error instanceof Error ? error.message : 'unknown error' });
  }
}
